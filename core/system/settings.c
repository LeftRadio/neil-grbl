...
/* Private Functions ---------------------------------------------------------*/
/**
  * @brief  Reads Grbl global settings struct from EEPROM.
  * @param  None
  * @retval uint8_t
  */
static uint8_t read_global_settings(void) {
...
/**
  * @brief  Method to store startup lines into EEPROM
  * @param  uint8_t n, char *line
  * @retval None
  */
...
/**
  * @brief  Method to store build info into EEPROM
...
/**
  * @brief  Method to store coord data parameters into EEPROM
  * @param  uint8_t coord_select, float *coord_data
  * @retval None
  */
...
/**
  * @brief  Method to store Grbl global settings struct and version number into EEPROM
            NOTE: This function can only be called in IDLE state.
  * @param  None
  * @retval None
  */
...
/**
  * @brief  Method to restore EEPROM-saved Grbl global settings back to defaults.
  * @param  None
  * @retval None
  */
void settings_restore(uint8_t restore_flag) {
    ...
    if (restore_flag & SETTINGS_RESTORE_STARTUP_LINES) {
          ...
          /* checksum */
          eeprom_put_char(EEPROM_ADDR_STARTUP_BLOCK + 1, 0);
        #endif
        #if N_STARTUP_LINE > 1
          eeprom_put_char(EEPROM_ADDR_STARTUP_BLOCK + (LINE_BUFFER_SIZE + 1), 0);
          /* checksum */
          eeprom_put_char(EEPROM_ADDR_STARTUP_BLOCK + (LINE_BUFFER_SIZE + 2), 0);
        #endif
    }
    /* */
    if (restore_flag & SETTINGS_RESTORE_BUILD_INFO) {
        ...
        /* checksum */
        eeprom_put_char(EEPROM_ADDR_BUILD_INFO + 1 , 0);
    }
}

/**
  * @brief  Reads startup line from EEPROM. Updated pointed line string data.
  * @param  uint8_t n, char *line
  * @retval uint8_t
  */
...
/**
  * @brief  Read selected coordinate data from EEPROM. Updates pointed coord_data value.
  * @param  uint8_t coord_select, float *coord_data
  * @retval uint8_t
  */
...
/**
  * @brief  A helper method to set settings from command line
  * @param  None
  * @retval uint8_t
  */
