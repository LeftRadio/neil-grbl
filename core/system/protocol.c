...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/* define line flags, includes comment type tracking
   and line overflow detection */
...
/**
  * @brief  Handles Grbl system suspend procedures, such as feed hold, safety door, and parking motion.
            The system will enter this loop, create local variables for suspend tasks, and return to
            whatever function that invoked the suspend, such that Grbl resumes normal operation.
            This function is written in a way to promote custom parking motions. Simply use this as a template
  * @param  None
  * @retval None
  */
static void protocol_exec_rt_suspend(void) {
  ...
  uint8_t restore_condition;
    ...
    #endif
  #else
    if (block == (void*)0) { restore_condition = (gc_state.modal.spindle | gc_state.modal.coolant); }
      ...
      if (sys.state & (STATE_SAFETY_DOOR | STATE_SLEEP)) {
        ...
        if (bit_isfalse(sys.suspend,SUSPEND_RETRACT_COMPLETE)) {
          ...
          if (sys.state == STATE_SLEEP) {
            ...
            spindle_set_state(SPINDLE_DISABLE,0.0); // De-energize
            coolant_set_state(COOLANT_DISABLE); // De-energize
            stepper_go_idle(); // Disable steppers
          ...
          if (sys.suspend & SUSPEND_INITIATE_RESTORE) {
            ...
            if (gc_state.modal.spindle != SPINDLE_DISABLE) {
              ...
              if (bit_isfalse(sys.suspend,SUSPEND_RESTART_RETRACT)) {
                if (bit_istrue(settings.flags,BITFLAG_LASER_MODE)) {
                  ...
                  delay_sec_nonblock(SAFETY_DOOR_SPINDLE_DELAY, DELAY_MODE_SYS_SUSPEND);
                }
              }
            }
            if (gc_state.modal.coolant != COOLANT_DISABLE) {
              ...
              if (bit_isfalse(sys.suspend,SUSPEND_RESTART_RETRACT)) {
                ...
                delay_sec_nonblock(SAFETY_DOOR_COOLANT_DELAY, DELAY_MODE_SYS_SUSPEND);
              }
            }
...
/**
  * @brief  Block until all buffered steps are executed or in a cycle state. Works with feed hold
...
/**
  * @brief  Auto-cycle start triggers when there is a motion ready to execute and if the main program is not
...
/**
  * @brief  This function is the general interface to Grbl's real-time command execution system. It is called
...
/**
  * @brief  Executes run-time commands, when required. This function primarily operates as Grbl's state
        ...
        if (!(sys.state & (STATE_ALARM | STATE_CHECK_MODE))) {
          ...
          if (sys.state & (STATE_CYCLE | STATE_JOG)) {
            if (!(sys.suspend & (SUSPEND_MOTION_CANCEL | SUSPEND_JOG_CANCEL))) { // Block, if already holding.
              stepper_update_plan_block_parameters(); // Notify stepper module to recompute for hold deceleration.
          ...
          if ((sys.state == STATE_IDLE) || ((sys.state & STATE_HOLD) && (sys.suspend & SUSPEND_HOLD_COMPLETE))) {
            if (sys.state == STATE_HOLD && sys.spindle_stop_ovr) {
              ...
              if (plan_get_current_block() && bit_isfalse(sys.suspend,SUSPEND_MOTION_CANCEL)) {
                ...
                stepper_prep_buffer(); // Initialize step segment buffer before beginning cycle.
                stepper_wake_up();
